...
def select_child():
    conn = get_db_conn()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT * FROM children WHERE parent_id=%s", (current_user.id,))
...
def dashboard():
    ...
    # --- Fetch child ---
    cursor.execute("SELECT * FROM children WHERE id=%s AND parent_id=%s", (child_id, current_user.id))
    ...
    # --- Academic scores ---
    cursor.execute("SELECT * FROM academic_scores WHERE child_id=%s ORDER BY date ASC", (child_id,))
    ...
    # --- AI Results (preschool, learning, tutoring) ---
    cursor.execute("""
    ...
    # Map results by module
    ai_map = {r['module']: r for r in ai_results}

    preschool_ai = ai_map.get("preschool", {}).get("result", "<p class='text-muted'>No preschool analysis yet.</p>")
    learning_ai = ai_map.get("learning", {}).get("result", "<p class='text-muted'>No learning style analysis yet.</p>")
    tutoring_ai = ai_map.get("tutoring", {}).get("result", "<p class='text-muted'>No tutoring recommendations yet.</p>")
...
def add_test():
    ...
    if not name or not question_texts:
        flash("Please enter a test name and at least one question.", "warning")
        return redirect(url_for("profile"))

    conn = get_db_conn()
    cursor = conn.cursor()

    cursor.execute("INSERT INTO tests (name, user_id) VALUES (%s, %s)", (name, current_user.id))
    test_id = cursor.lastrowid

    for q_text, q_type in zip(question_texts, question_types):
        cursor.execute(
            "INSERT INTO test_questions (test_id, question, answer_type) VALUES (%s, %s, %s)",
            (test_id, q_text, q_type)
        )
...
@app.route("/tests/edit/<int:test_id>", methods=["POST"])
def edit_test(test_id):
    ...
    cursor.execute(
        "UPDATE tests SET name=%s WHERE id=%s AND user_id=%s",
        (name, test_id, current_user.id),
    )
...
def delete_test(test_id):
    conn = get_db_conn()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM tests WHERE id=%s AND user_id=%s", (test_id, current_user.id))
    conn.commit()
    cursor.close()
    conn.close()
...
def profile():
    ...
    cursor.execute("SELECT * FROM tests WHERE user_id=%s", (current_user.id,))
    tests_raw = cursor.fetchall()

    tests = []
    for t in tests_raw:
        cursor.execute(
            "SELECT question, answer_type FROM test_questions WHERE test_id=%s ORDER BY id ASC",
            (t["id"],)
        )
        questions = cursor.fetchall() 
        tests.append({"id": t["id"], "name": t["name"], "questions": questions})

    cursor.close()
    conn.close()

    return render_template("profile.html", user=user, children=children, tests=tests)
...
def edit_child(child_id):
    ...
    """, (name, dob, age, grade_level, gender, notes, child_id, current_user.id))
    conn.commit()
    cursor.close(); conn.close()
...
def academic_progress():
    ...
    if request.method == "POST":
        ...
        if subject and score and year and month:
            record_date = date(year, month, 1)
            cursor.execute(
                "INSERT INTO academic_scores (child_id, subject, score, date) VALUES (%s, %s, %s, %s)",
...
def preschool_tracker():
    ...
    child_info = f"Name: {child['name']}, Gender: {child['gender']}, " \
    ...
    if request.method == "POST" and "domain" in request.form:
        ...
        cursor.execute(
            "INSERT INTO preschool_assessments (child_id, domain, description, date) VALUES (%s, %s, %s, %s)",
    ...
    # If no data, skip Gemini
    if not assessments:
        benchmark_summary = "<p class='text-muted'>No preschool milestone data available yet. Add some assessments to view AI analysis.</p>"
        ...
        if ai_row and ai_row.get("data") == data_payload:
            ...
            # --- Run Gemini only when we have data ---
            try:
                combined_text = "\n".join(
                    [f"- {a['domain']}: {a['description']} (at {a['age_months']} months)" for a in assessments]
                )
                ...
                benchmark_summary = f"(Error generating analysis: {e})"
...
def regenerate_preschool_ai():
    ...
    # Fetch preschool assessments
    cursor.execute("SELECT * FROM preschool_assessments WHERE child_id=%s ORDER BY date DESC", (child_id,))
    assessments = cursor.fetchall()

    if not assessments:
        cursor.close()
        conn.close()
        return jsonify({"error": "No assessments found"}), 400

    # Prepare data for Gemini
    combined_text = "\n".join(
        [f"- {a['domain']}: {a['description']} (at {a['date']})" for a in assessments]
    )
    prompt = f"""
    You are an early childhood development expert.
    ...
    # Save or update existing ai_results entry
    cursor.execute(
        "SELECT id FROM ai_results WHERE child_id=%s AND module='preschool'",
        (child_id,)
    )
    ...
    if existing:
        cursor.execute(
            ...
            WHERE id=%s
            """,
            (data_payload, benchmark_summary, existing["id"])
        )
    else:
        cursor.execute(
            """
...
def delete_preschool(id):
    ...
    cursor.execute("DELETE FROM preschool_assessments WHERE id = %s", (id,))
    conn.commit()
...
def learning_style():
    ...
    if request.method == "POST" and "observation" in request.form:
        ...
        cursor.execute(
            "INSERT INTO learning_observations (child_id, observation, created_at) VALUES (%s, %s, NOW())",
            (child_id, observation)
        )
        conn.commit()
        return redirect(url_for("learning_style"))

    # --- Fetch tests available for this user ---
    cursor.execute("SELECT * FROM tests WHERE user_id=%s", (current_user.id,))
    user_tests = cursor.fetchall()

    # --- Fetch observations ---
    cursor.execute("SELECT * FROM learning_observations WHERE child_id=%s ORDER BY created_at DESC", (child_id,))
    learning_notes = cursor.fetchall()

    # --- Fetch test answers + related questions ---
    cursor.execute("""
        SELECT 
            ta.id AS answer_id,
            ta.test_id,
    ...
    # --- Combine and serialize payload for caching check ---
    ...
    # --- Run Gemini only if new data or regen ---
    if (regen == "1" or not use_cached) and (learning_notes or test_answers):
        try:
            obs_text = "\n".join([f"- {o['observation']}" for o in learning_notes]) if learning_notes else "No observations."

            # Group answers by test for structure
            test_grouped = {}
            ...
            for test_name, answers in test_grouped.items():
                ...
                for a in answers:
                    ...
                    if str(answer_val).isdigit():
                        ans_text += f"  - Q: {question_text}\n    A: {answer_val} (on scale 1–5)\n"
                    else:
                        ans_text += f"  - Q: {question_text}\n    A: {answer_val}\n"
            ...
            if "token" in str(e).lower():
                return jsonify({"error": "token_limit"})
            benchmark_summary = f"<p class='text-danger'>(Error generating analysis: {e})</p>"

    elif not learning_notes and not test_answers:
        benchmark_summary = "<p class='text-muted'>No data available yet. Add observations or test answers first.</p>"
    ...
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return jsonify({
            "benchmark_summary": benchmark_summary,
            "last_generated": last_generated.strftime("%Y-%m-%d %H:%M:%S") if last_generated else None,
            "use_cached": use_cached
        })
...
def get_test_questions(test_id):
    ...
    # ensure test belongs to this user
    cursor.execute("SELECT * FROM tests WHERE id=%s AND user_id=%s", (test_id, current_user.id))
...
def take_learning_test(child_id):
    ...
    for key in request.form:
        if key.startswith("answers[") and key.endswith("[answer]"):
            # Extract index from key, e.g., answers[0][answer] -> 0
            ...
            if question_id and answer_value is not None:
                cursor.execute(
                    """
                    INSERT INTO test_answers (child_id, test_id, question_id, answer, created_at)
                    VALUES (%s, %s, %s, %s, NOW())
                    """,
...
def submit_learning_observation(child_id):
    ...
    if not observation:
        ...
        return redirect(url_for("learning_style", child_id=child_id))
...
def tutoring_recommendations():
    ...
    # --- Fetch learning + preschool AI results ---
    cursor.execute("SELECT module, result, updated_at FROM ai_results WHERE child_id=%s AND module IN ('learning', 'preschool')", (child_id,))
    child_ai = cursor.fetchall()

    learning_result = next((r["result"] for r in child_ai if r["module"] == "learning"), None)
    preschool_result = next((r["result"] for r in child_ai if r["module"] == "preschool"), None)

    # --- Fetch tutoring cache ---
    cursor.execute("SELECT * FROM ai_results WHERE child_id=%s AND module='tutoring' LIMIT 1", (child_id,))
    cached = cursor.fetchone()

    # --- Handle regeneration trigger ---
    regen = request.args.get("regen")

    data_payload = json.dumps({"learning": learning_result, "preschool": preschool_result}, ensure_ascii=False, indent=2)
    use_cached = cached and cached["data"] == data_payload and not regen
    tutoring_summary = cached["result"] if use_cached else None
    last_generated = cached["updated_at"] if cached else None

    # --- Skip if no data from other modules ---
    if not learning_result and not preschool_result:
        tutoring_summary = "<p class='text-muted'>No AI analysis data available yet. Run preschool and learning style assessments first.</p>"

    # --- Run Gemini only if needed and we have data ---
        ...
        try:
            ...
            if "token" in str(e).lower():
                return jsonify({"error": "token_limit"})
            tutoring_summary = f"<p class='text-danger'>(Error generating recommendations: {e})</p>"

    cursor.close(); conn.close()

    # --- Handle AJAX for regen button ---
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return jsonify({
...
@login_required
def ai_insights():
    # Use the currently selected child from session
    child_id = session.get("selected_child")
    if not child_id:
    ...
    # --- Fetch child info (and make sure it belongs to this parent) ---
    cursor.execute(
    ...
    # --- Fetch academic scores for this child ---
    cursor.execute(
        "SELECT subject, score, date FROM academic_scores WHERE child_id=%s ORDER BY date ASC",
        (child_id,),
    )
    scores = cursor.fetchall()

    # --- Simple rule-based strengths / weaknesses (keep this!) ---
    strengths = []
    weaknesses = []
    ...
    # --- Prepare payload for caching in ai_results ---
    ...
    regen = request.args.get("regen")  # /dashboard/insights?regen=1
    use_cached = cached and cached["data"] == data_payload and not regen

    ai_summary = None
    last_generated = cached["updated_at"] if cached else None

    # --- Use cached summary if data didn't change and no regen requested ---
    if use_cached:
        ai_summary = cached["result"]

    else:
        # --- Call Gemini to generate a natural-language summary ---
        if scores:  # only if we have some scores
            try:
                # Create a compact list of "subject: score" lines
                score_lines = [
...
"""
                ...
                # If AI fails, log it and continue with rule-based info only
                app.logger.error(f"AI insights error: {e}")
                ai_summary = None
        else:
            ai_summary = None  # no scores to analyze

    cursor.close()
    conn.close()

    # Render inside the main dashboard layout using a partial
    return render_template(
        "dashboard.html",
...
def learning_plan():
    ...
    # --- Fetch child info (and ensure it belongs to this parent) ---
    cursor.execute(
        "SELECT * FROM children WHERE id=%s AND parent_id=%s",
        (child_id, current_user.id),
    )
    ...
    # Build a compact text version for the prompt
    score_lines = []
    for row in scores:
        if row["score"] is not None:
            score_lines.append(f"{row['subject']}: {row['score']}")
    scores_text = "\n".join(score_lines) if score_lines else "No academic scores yet."

    # --- Existing AI outputs from other modules (if available) ---
    cursor.execute(
        """
    ...
    learning_result = next((r["result"] for r in other_ai if r["module"] == "learning"), None)
    preschool_result = next((r["result"] for r in other_ai if r["module"] == "preschool"), None)
    tutoring_result = next((r["result"] for r in other_ai if r["module"] == "tutoring"), None)
    ...
    regen = request.args.get("regen")  # /dashboard/plan?regen=1
    use_cached = cached and cached["data"] == data_payload and not regen

    plan_html = None
    last_generated = cached["updated_at"] if cached else None

    # --- Use cached plan if data is unchanged ---
    if use_cached:
        plan_html = cached["result"]

    else:
        # Only talk to Gemini if we have at least some information
...
<table class="table table-striped">
  ...
  <tbody>
    <!-- For each day Monday–Sunday, create 2–4 activities -->
    <!-- Example row:
    <tr>
      <td>Monday</td>
      <td>
        <ul>
          <li>Practice English reading for 10 minutes using picture books.</li>
          <li>Spend 10 minutes on simple counting games with toys.</li>
        </ul>
      </td>
    </tr>
    -->
...
def resources_hub():
    ...
    # --- Fetch child info (and ensure it belongs to this parent) ---
    cursor.execute("SELECT * FROM children WHERE id=%s AND parent_id=%s",
                   (child_id, current_user.id))
    child = cursor.fetchone()
    if not child:
        cursor.close(); conn.close()
    ...
    # --- Academic scores (for subject needs) ---
    cursor.execute("""
    ...
    # --- Learning style AI result (if any) ---
    cursor.execute("""
        SELECT result
        FROM ai_results
        WHERE child_id=%s AND module='learning'
    ...
    # Payload used to decide if cache is still valid
    context_data = {
        "scores": scores,
    ...
    # --- Check existing cached resources plan ---
    cursor.execute("""
        SELECT *
        FROM ai_results
    ...
    # --- If no cache or data changed, call Gemini ---
    if not use_cached:
        if not scores and not learning_summary:
            ...
            # Build compact text of scores
            score_lines = [
                f"{row['subject']}: {row['score']}"
                for row in scores if row.get("score") is not None
            ]
...
"""
            ...
            try:
                ...
                # Save or update cache in ai_results
                if cached:
                    cursor.execute("""
                        UPDATE ai_results
                        SET data=%s, result=%s, updated_at=NOW()
                ...
                if not resources_html:
                    resources_html = "<p class='text-danger'>Unable to generate resources at the moment.</p>"
